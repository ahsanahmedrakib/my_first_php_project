<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
     ...
     $id =   $_POST['id'];
    $select = "SELECT * FROM `people_content` WHERE id = '$id' ";
    ...
    if (isset($_FILES['image']['name']) && isset($_FILES['image']['name']) !== null && !empty($_FILES['image']['name'])) {
      ...
      if (file_exists($fileurl)) 
        ...
        $update = "UPDATE `people_content` SET `name` = '$name', `designation` = '$designation',`description` = '$description', `image` = '$file_name' WHERE `id` = '$id' ";
    }else{
                // echo 'this is if else'; exit();

        $update = "UPDATE `people_content` SET `name` = '$name', `designation` = '$designation',`description` = '$description' WHERE `id` = '$id' ";
    }
