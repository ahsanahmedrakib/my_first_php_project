<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
     ...
     $id =   $_POST['id'];
    $select = "SELECT * FROM `portfolio_content` WHERE id = '$id' ";
    ...
    if (isset($_FILES['image']['name']) && isset($_FILES['image']['name']) !== null && !empty($_FILES['image']['name'])) {
      ...
      if (file_exists($fileurl)) 
        ...
        $update = "UPDATE `portfolio_content` SET `title` = '$title', `url` = '$url',`class` = '$class', `image` = '$file_name' WHERE `id` = '$id' ";
    }else{
                // echo 'this is if else'; exit();

        $update = "UPDATE `portfolio_content` SET `title` = '$title', `url` = '$url',`class` = '$class' WHERE `id` = '$id' ";
    }
